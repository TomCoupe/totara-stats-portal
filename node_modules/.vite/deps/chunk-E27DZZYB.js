      ...
      return [r].flat().reduce((s3, a2) => {
        if (a2 != null) {
          ...
          if (u === "string" || u === "number") s3.push(a2);
            ...
            let c = Array.isArray(a2) ? o(n, a2) : Object.entries(a2).map(([f, g3]) => n === "style" && (g3 || g3 === 0) ? `${f.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase()}:${g3}` : g3 ? f : void 0);
            s3 = c.length ? s3.concat(c.filter((f) => !!f)) : s3;
          }
        }
        return s3;
      }, i2);
  ...
  if (p(e)) {
    ...
    if (z(a2, k)) {
      let n = a2.replaceAll(k, (l2) => {
        let c = l2.replace(/{|}/g, "").split(".").filter((m2) => !s3.some((d2) => z(m2, d2)));
...
function le(e, t) {
  ...
  function r(n, l2) {
    ...
    for (; c <= n.length; ) {
      let g3 = n[c];
      if ((g3 === '"' || g3 === "'" || g3 === "`") && n[c - 1] !== "\\" && (d2 = d2 === g3 ? null : g3), !d2 && (g3 === "(" && u++, g3 === ")" && u--, (g3 === "," || c === n.length) && u === 0)) {
        let f = m2.trim();
  ...
  for (let n = i2.length - 1; n >= 0; n--) {
    let [l2, o] = i2[n], c = e.slice(l2 + 3, o), m2 = r(c, t), d2 = t(...m2);
...
function de(e, t = {}) {
  ...
  for (; o.length; ) {
    ...
    for (let u in m2) {
      let g3 = m2[u], f = ve(g3), p3 = z(u, a2) ? Q(d2) : Q(d2, ee(u));
...
var b2 = { regex: { rules: { class: { pattern: /^\.([a-zA-Z][\w-]*)$/, resolve(e) {
  ...
  var R, T2, j, O, M, z3, V;
  let { preset: a2, options: n } = t, l2, o, c, m2, d2, u, g3;
  ...
  var c;
  let n = { name: e, theme: t, params: r, set: i2, defaults: a2 }, l2 = (c = e.includes("-directive") ? this.getPresetD(n) : this.getPresetC(n)) == null ? void 0 : c.css, o = Object.entries(s3).reduce((m2, [d2, u]) => m2.push(`${d2}="${u}"`) && m2, []).join(" ");
  ...
  let a2 = function(l2, o = {}, c = []) {
    ...
    Object.entries(l2).forEach(([m2, d2]) => {
      let u = z(m2, t.variable.excludedKeyRegex) ? o : o ? `${o}.${oe2(m2)}` : oe2(m2), g3 = c ? `${c}.${m2}` : m2;
      i(d2) ? n(d2, u, g3) : (i2[u] || (i2[u] = { paths: [], computed: (f, x2 = {}, p3 = []) => {
  ...
  return n.length === 1 ? n[0].value : n.reduce((o = {}, c) => {
    let u = c, { colorScheme: m2 } = u, d2 = v(u, ["colorScheme"]);
    return o[m2] = d2, o;
  }, void 0);
  ...
  if (s(t)) {
    ...
    if (s3 !== "style") {
      ...
      t = r === "dark" ? c.reduce((m2, { type: d2, selector: u }) => (s(u) && (m2 += u.includes("[CSS]") ? u.replace("[CSS]", t) : this.getSelectorRule(u, l2, d2, t)), m2), "") : C(l2 != null ? l2 : ":root,:host", t);
    }
    if (o) {
...
//# sourceMappingURL=chunk-E27DZZYB.js.map
